...
const EmployeeDashboard = () => {
  ...
  const fetchMyTasks = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}`}'}/tasks/user/${user.empId}`);
      if (res.ok) {
  ...
  const handleTaskStatusUpdate = async (taskId, newStatus) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}`}'}/project-tasks/${taskId}`, {
        method: 'PUT',
