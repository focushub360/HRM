...
const LiveTracking = () => {
    ...
    useEffect(() => {
        const fetchMyEmployees = async () => {
            ...
            // If I am 'hr', I filter.
            let url = `${import.meta.env.VITE_API_URL || (import.meta.env.DEV ? 'http://localhost:5000/api' : 'https://hrms-backend-22uq.onrender.com/api')}/companies/${user.companyId}/employees`;
            if (user.type === 'hr') {
                url += `?hrId=${user.id}`;
            }
