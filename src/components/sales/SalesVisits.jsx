...
const SalesVisits = () => {
    ...
    const fetchStats = async () => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}`}'}/gps/stats/${selectedEmployee}?date=${selectedDate}`);
            if (res.ok) {
    ...
    const fetchEmployees = async () => {
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}`}'}/companies/${user.companyId}/employees`);
    ...
    const fetchVisits = async (showLoader = false) => {
        ...
        try {
            let url = `${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}`}'}/visits/${user.companyId}`;
    ...
    const fetchRoute = async (showLoader = false) => {
        if (!selectedEmployee) return;
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}`}'}/gps/route/${selectedEmployee}`);
            if (res.ok) {
    ...
    const handleDeleteVisit = async (visitId) => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}`}'}/visits/${visitId}`, {
                method: 'DELETE'
            });
            if (res.ok) {
