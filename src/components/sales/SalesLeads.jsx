...
const SalesLeads = () => {
    ...
    const fetchLeads = async (showLoader = false) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://hrms-backend-22uq.onrender.com/api'}/leads/${user.companyId}`);
            if (response.ok) {
    ...
    const updateLeadData = async (id, data) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://hrms-backend-22uq.onrender.com/api'}/leads/${id}`, {
                method: 'PUT',
